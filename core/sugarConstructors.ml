...
module SugarConstructors (Position : Pos)
  ...
  (* Create a database insertion query.  Raises an exception when the list of
     ...
     *)
  let db_insert ?(ppos=dp) mode ins_exp lbls exps var_opt =
    if is_empty_db_exps exps && var_opt == None then
      raise (ConcreteSyntaxError (pos ppos, "Invalid insert statement. " ^
          ...
          "variable."));
    with_pos ppos (DBInsert (mode, ins_exp, lbls, exps,
  ...
  let table_lit ?(ppos=dp) name record_type field_constraints keys temporal_metadata database =
    let temporal_metadata =
      OptionUtils.from_option (TemporalMetadata.current false) temporal_metadata in
