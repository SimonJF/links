...
const LINKS = new (function() {
    ...
    while (queue.length > 0) {
      ...
      if (obj instanceof Object) {
        ...
        for (let i in obj) {
          ...
          } else if (obj[i].hasOwnProperty("_dateTime")) {
              // DateTimes are serialised via a UTC UNIX timestamp.
              // If not infinity / negative infinity, we need to install
              // a "date" field which includes a JS Date object.
              // (In turn, this will be removed before sending to the server).
              const dt = obj[i]["_dateTime"];

              if (dt["type"] == "timestamp") {
                  // Kinda annoyingly, the date constructor has to be in local
                  // time (and is converted to UTC under the hood).
                  const tzOffset = new Date().getTimezoneOffset() * -1 * 60 * 1000;
                  obj[i]["_dateTime"]["value"] = new Date(dt["value"] * 1000 + tzOffset);
              }
          }
        }
      }
  ...
  function replacer (key, value) {
    ...
    } else if (key == "_dateTime") {
      // If date is set, then we need to serialise the UTC timestamp
      if (value["date"]) {
        const ts = Math.floor(value["value"].getTime() / 1000);
...
function _show(obj) {
  if (obj.hasOwnProperty("_dateTime")) {
...
function projectDate(dt) {
    if (dt["_dateTime"]["type"] == "timestamp") {
        DEBUG.assert(dt["_dateTime"]["value"] instanceof Date);
        return dt["_dateTime"]["value"];
    } else {
...
function showLocalDate(dt) {
    switch (dt["_dateTime"]["type"]) {
        case "infinity":
        case "-infinity":
            return dt["_dateTime"]["type"];
        case "timestamp":
            const ts = dt["_dateTime"]["value"];
            ...
            const day = ts.getDay().toString().padStart(2, '0');
...
/* Constants */
const forever = { _dateTime: { type: "infinity", value: -1 } };
const beginning_of_time = { _dateTime: { type: "-infinity", value: -1 } };
...
function _dateToInt(dt) {
  ...
  const tzOffset = new Date().getTimezoneOffset() * -1 * 60 * 1000;
  return Math.floor((dt["_dateTime"]["value"].getTime() - tzOffset) / 1000);
}
...
function _now() {
    return { _dateTime: { type: "timestamp", value: new Date() } };
}
...
function _showUTC(dt) {
    switch (dt["_dateTime"]["type"]) {
        ...
        case "timestamp":
            ...
            const month = ts.getUTCMonth().toString().padStart(2, '0');
            const day = ts.getUTCDay().toString().padStart(2, '0');
...
const dateHour = LINKS.kify(_dateHour);
const dateMinute = LINKS.kify(_dateMinute);
const dateSecond = LINKS.kify(_dateSecond);
