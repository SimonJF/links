...
function _handleSessionException(cont_variables) {
  ...
  const resolved_cont_set = cancelValueSet(cont_set);
  const affected_channels = Array.from(resolved_cont_set);
  _debug("Handling session exception. Affected channels:");
...
function _cancel(c) {
  ...
  function cancelContainedValues() {
    ...
    const chans = Array.from(cancelValueSet(buf_set));
...
/* Cancels an arbitrary value */
function cancelValue(obj) {
  cancelValueSet(new Set(obj));
}
