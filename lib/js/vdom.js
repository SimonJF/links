...
// SJF: Not entirely sure what these do yet.
let fNames = [];
let subEvents = {};
...
/**
 * evalAttr: Evaluates a Links MvuAttr.Attr, resulting in a list of
 * plain HTML attributes and event handlers to install.
 *
 * Argument: `attr`, a Links variant of type "MvuAttr.Attr".
...
function evalAttr(attr) {
  ...
  if (lbl == "MvuAttr.AttrEmpty") {
    // Nothing doing.
    return makeRes([], []);
  } else if (lbl == "MvuAttr.AttrAppend") {
    ...
    return makeRes(
      a1Res.attributes.concat(a2Res.attributes),
      a1Res.eventHandlers.concat(a2Res.eventHandlers));
  } else if (lbl == "MvuAttr.AttrAttribute") {
    return makeRes([{ key: val["1"], value: val["2"] }], []);
  } else if (lbl == "MvuAttr.AttrEventHandler") {
    return makeRes([], [val])
  } else {
...
function evalHTML(html) {
  ...
  if (lbl == "MvuHTML.HTMLEmpty") {
    ...
    const attrRes = evalAttrs(val.attrs);
    ...
    const evtHandlerAttrs = setupEventHandlers(attrs, evtHandlers);
    // Add the event handlers to the generated attributes.
    attrs.concat(evtHandlerAttrs);
    const children = evalHTML(val.children);
    return [h(val["tagName"], attrs, children)];
  } else {
