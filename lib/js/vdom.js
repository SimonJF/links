 ...
 * Result: { windowEvents: { eventName: {key: unique key, callback: callback} },
 *           intervalEvents: [(Interval, callback)],
 *           animationFrameHandler: either `null` or a handler function }
 *
...
function evalSubscription(subscription) {
  ...
  function evalSubscriptionInner(subscription) {
    ...
    if (tag == "MvuSubscriptions.SubEmpty") {
      ...
      // TODO: Fill me in. Ideally we'll want to abstract over the
      // event handler setup we have elsewhere.
...
// Top-level node must be a tree instead of a forest, so
// wrap it in a div.
